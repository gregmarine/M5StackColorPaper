...
bool is_indexed_bmp_file(const char *path)
{
    ...
    if (!f) return false;
    // BITMAPFILEHEADER (14) + start of BITMAPINFOHEADER: biBitCount is at offset 28.
    uint8_t hdr[30];
    size_t n = fread(hdr, 1, sizeof(hdr), f);
    fclose(f);
    if (n < sizeof(hdr) || hdr[0] != 'B' || hdr[1] != 'M') return false;
    uint16_t bit_count = (uint16_t)(hdr[28] | (hdr[29] << 8));
    return bit_count == 1 || bit_count == 4 || bit_count == 8;
}
