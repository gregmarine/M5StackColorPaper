...
void Hal::init()
{
    ...
    // 24-bit canvas (lives in PSRAM, 600*400*3 = 720 KB). The panel driver's
    // framebuffer is RGB888; a 16-bit sprite would put every pixel through a
    // 565 round-trip on pushSprite, which perturbs the exact palette colours
    // that pre-dithered photos rely on for 1:1 mapping to panel colours.
...
void Hal::settingsInit()
{
    ...
    // The upstream demo rotated the *canvas* here to honour the saved
    // landscape/portrait preference. This firmware instead rotates the panel
    // itself per image (see PhotoSlideshow::displayPhoto) and keeps the canvas
    // an unrotated 1:1 copy of the panel, so the setting is deliberately not
    // applied. (A rotated sprite keeps its rotation across createSprite(),
    // which made every photo come out on its side.)
